...
# ensure that outputs are in
# ${workflow.launchDir}/outputs/SMR/${qtl_type}/${pheno_prefix}
# SAVE TO ->
# ${workflow.launchDir}/outputs/SMR/res/${qtl_type}/${pheno1_prefix}_${pheno2_prefix}
# Dir structure
# genes_passed_fdr.tsv
# genes_final.tsv
# ${qtl_type}_genes.txt
# per QTL dataset

def analyse_qtls(smr_res_trait1: str,smr_res_trait2: str,pheno1_prefix: str,pheno2_prefix: str,gene_list_path: str,out_dir: str,heidi_thresh: float,qtl_type: str):
    pd.set_option("display.float_format", "{:.10f}".format)
    t1 = pd.read_csv(smr_res_trait1, sep="\t")
    t2 = pd.read_csv(smr_res_trait2, sep="\t")
    required_cols = {"qtl_name","index","p_SMR_multi","p_HEIDI","b_SMR","se_SMR","p_SMR","A1","A2","topSNP","topSNP_chr","topSNP_bp","probeID","gene_id","ProbeChr","Probe_bp","Freq","nsnp_HEIDI"}
    miss1 = required_cols - set(t1.columns)
    miss2 = required_cols - set(t2.columns)
    if miss1: raise ValueError(f"{smr_res_trait1} missing columns: {sorted(miss1)}")
    if miss2: raise ValueError(f"{smr_res_trait2} missing columns: {sorted(miss2)}")
    genes = (pd.read_csv(gene_list_path, header=None, sep=None, engine="python").iloc[:,0].astype(str).str.strip().str.upper().tolist())
    ...
    keep_cols = ["GENE","index","gene_id","qtl_name","probeID","ProbeChr","Probe_bp","topSNP","topSNP_chr","topSNP_bp","A1","A2","Freq","b_SMR","se_SMR","p_SMR","p_SMR_multi","q_SMR_multi","p_HEIDI","nsnp_HEIDI","trait"]
    def prep(df, qtl):
    ...
    for qtl in qtls:
        ...
        keep_fdr = (pd.to_numeric(m["q_SMR_multi_A"], errors="coerce") < 0.05) & (pd.to_numeric(m["q_SMR_multi_S"], errors="coerce") < 0.05) & ((pd.to_numeric(m["p_HEIDI_A"], errors="coerce") > 0) | (pd.to_numeric(m["p_HEIDI_S"], errors="coerce") > 0))
        passed = m[keep_fdr].copy()
        keep_final = keep_fdr & (pd.to_numeric(m["p_HEIDI_A"], errors="coerce") > float(heidi_thresh)) & (pd.to_numeric(m["p_HEIDI_S"], errors="coerce") > float(heidi_thresh))
...
def main():
    ...
    a = p.parse_args()
    analyse_qtls(a.smr_res_trait1,a.smr_res_trait2,a.pheno1_prefix,a.pheno2_prefix,a.gene_list,a.out_dir,a.heidi_thresh,a.qtl_type)
